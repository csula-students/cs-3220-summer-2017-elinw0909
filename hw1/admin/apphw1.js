...
class Cart {
    ...
    renderListAsHTML(list) {
        ...
        for(var i = 0; i < this.store.cartItems.length; i ++){
            ...
            <td>${this.store.cartItems[i].name }</td>
            <td>${this.store.cartItems[i].price}</td>
            <td>${this.store.cartItems[i].quantity}</td>
...
class Menu {
    ...
    render () {
        ...
        let tbody = this.root.querySelector('tbody');
        tbody.innerHTML = this.renderHTMLForCart(foods);
...
renderHTMLForCart(foods){
        let table ='';
        for(var i =0; i <this.store.foods.length; i ++){
             table += `<tr>
           
            <td>${this.store.foods[i].name }</td>
            <td>${this.store.foods[i].iamge}</td>
            <td>${this.store.foods[i].ingredients}</td>
            <td>${this.store.foods[i].description}</td>
            <td>${this.store.foods[i].price}</td>
            <td><button class = "delete-button">Delete</button></td></tr>`;
        }
        return table;
}
...
class CreateFood {
    ...
    foods.push({
        ...
        ingredients:this.root.dataset.ingredients,
...
// the end of document
 document.addEventListener('DOMContentLoaded', () => {
    ...
    let cart = document.querySelector('.cart-table');
    let checkoutButtons = document.querySelectorAll('.checkout-button');
    let createFood = document.querySelector('.create-food-table');
    let inventory = document.querySelector('.inventory-table');
    let menu = document.querySelector('.menu-table');
